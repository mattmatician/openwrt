...
static int qca807x_config(struct phy_device *phydev)
{
	...
	if (phy_read(phydev, QCA807X_CHIP_CONFIGURATION)) {
		...
		if (of_property_read_bool(node, "qcom,fiber-enable")) {
			...
			fiber_mode_autodect = phy_read_mmd(phydev, MDIO_MMD_AN, QCA807X_MMD7_FIBER_MODE_AUTO_DETECTION);
			fiber_mode_autodect |= QCA807X_MMD7_FIBER_MODE_AUTO_DETECTION_EN;
			phy_write_mmd(phydev, MDIO_MMD_AN, QCA807X_MMD7_FIBER_MODE_AUTO_DETECTION, fiber_mode_autodect);
			...
			chip_config |= FIELD_PREP(QCA807X_CHIP_CONFIGURATION_MODE_CFG_MASK, QCA807X_CHIP_CONFIGURATION_MODE_PSGMII_FIBER);
